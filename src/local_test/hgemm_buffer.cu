...
__global__ void wmma_kernel_double_buffered(TIN *a, TIN *b, TOUT *c, int M_PAD, int N_PAD, int K_PAD) {
    ...
    wmma::fragment<wmma::matrix_a, M_TILE, N_TILE, K_TILE, TIN, wmma::row_major> a_frag[2];
    wmma::fragment<wmma::matrix_b, M_TILE, N_TILE, K_TILE, TIN, wmma::row_major> b_frag[2];
    ...
    // Load first tile
    int current = 0;
    TIN *a_unique = a + midx * M_TILE * kdim * K_TILE;
    TIN *b_unique = b + nidx * N_TILE;
    
    wmma::load_matrix_sync(a_frag[current], a_unique, K_PAD);
    wmma::load_matrix_sync(b_frag[current], b_unique, N_PAD);
    
    for(int kidx = 0; kidx < kdim; kidx++) {
        // Prefetch next tile if not last iteration
        if (kidx + 1 < kdim) {
            int next = 1 - current;
            TIN *a_next = a_unique + K_TILE;
            TIN *b_next = b_unique + kidx * K_TILE * ndim * N_TILE + K_TILE * ndim * N_TILE;
            
            wmma::load_matrix_sync(a_frag[next], a_next, K_PAD);
            wmma::load_matrix_sync(b_frag[next], b_next, N_PAD);
        }
        
        // Compute with current tile
        wmma::mma_sync(c_frag, a_frag[current], b_frag[current], c_frag);
...
void GEMM_wmma_double_buffered(int M, int N, int K, TIN *a_in, TIN *b_in, TOUT *c_out) {
    assert(M != 0 && N != 0 && K != 0);
    ...
    Timer("gemm_gty_wmma_double_buffered", [&] {
        wmma_kernel_double_buffered<TGEMMIN, TGEMMOUT, M_TILE, N_TILE, K_TILE>
...
int main(int argc, char** argv) {
    ...
    // Test both versions
    GEMM_wmma_double_buffered<float, float>(M, N, K, h_a.data(), h_b.data(), h_c_wmma.data());

    cublas_gemm(M, N, K, h_a.data(), h_b.data(), h_c_cublas.data());

    float rel_error = compute_rel_error(h_c_wmma, h_c_cublas);
    std::cout << "Relative error between WMMA (double buffered) and cuBLAS results: " << rel_error << std::endl;
